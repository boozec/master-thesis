...
\begin{frame}{Cubbit -- How it works}
    \begin{columns}[c]
        \begin{column}{0.4\textwidth}
            Each file is split into $n+k$ shards (i.e., pieces) using
            Reed-Solomon. Each shard is sent to a different node (i.e., agent). \\ At least $n$ shards are
    ...
    \begin{columns}[c]
        \begin{column}{0.4\textwidth}
            \begin{enumerate}
                ...
                \item<2-> During upload, some nodes may be offline, but they might be online during verification.
